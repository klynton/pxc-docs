# State Snapshot Transfer (SST) Method for Percona XtraDB Cluster (PXC) Using Clone

--8<--- "tech-preview.md:4:4"

## SST Method: Clone

The Clone SST is a modern and efficient method that leverages MySQL's native cloning capabilities to transfer data from a donor node to a Joiner node. It is faster and more resource-efficient compared to traditional methods like xtrabackup or rsync.

## Limitations

Clone limitations are described in [Clone plugin limitations](clone-plugin-limitations.html)
...
To enable the `clone` SST method, ensure the [`wsrep_sst_allowed_methods`](wsrep-system-index.md#wsrep_sst_allowed_methods) variable in the configuration file (`my.cnf`) includes the `clone` method for both the donor and Joiner servers. This is essential for a successful State Snapshot Transfer (SST).
...
On the Joiner server, set the `wsrep_sst_method` variable to `clone` in the configuration file (`my.cnf`). This is the only accepted value for the Clone SST process.
...
For Percona XtraDB Cluster, any variables related to the SST mechanism, such as `wsrep_sst_allowed_methods` and `wsrep_sst_method`, must be defined prior to server startup to ensure proper synchronization.
...
### Common variables for SST methods

State Snapshot Transfer (SST) in Galera Cluster relies on specific variables that control its configuration and behavior. These variables must be set appropriately to ensure seamless synchronization between nodes during the SST process. Below is a list of the most commonly used variables and their purposes.

* **`sst-idle-timeout`**:  
  Sets the maximum time (in seconds) the SST process can remain idle before being considered failed.  
  [Learn more](wsrep-system-index.md#sst-idle-timeout)

* **`wsrep_sst_auth`**:  
  Provides the authentication credentials (e.g., username:password) required for SST.  
  [Learn more](wsrep-system-index.md#wsrep_sst_auth)

* **`wsrep_sst_compression`**:  
  Enables or disables data compression during the SST process to optimize bandwidth usage.  
  [Learn more](wsrep-system-index.md#wsrep_sst_compression)

* **`wsrep_sst_donor`**:  
  Defines the preferred donor node for SST. If not specified, the cluster automatically selects a donor.  
  [Learn more](wsrep-system-index.md#wsrep_sst_donor)

* **`wsrep_sst_donor_rejects_queries`**:  
  Determines whether the donor node should reject queries during SST to reduce resource usage and ensure transfer consistency.  
  [Learn more](wsrep-system-index.md#wsrep_sst_donor_rejects_queries)

* **`wsrep_sst_max_ws_size`**:  
  Configures the maximum size of the write-sets sent during SST.  
  [Learn more](wsrep-system-index.md#wsrep_sst_max_ws_size)

* **`wsrep_sst_method`**:  
  Specifies the method or script used for the State Snapshot Transfer (SST) process. Only one value can be selected.  
  [Learn more](wsrep-system-index.md#wsrep_sst_method)

* **`wsrep_sst_receive_address`**:  
  Specifies the IP address and port on the Joiner node to receive SST data.  
  [Learn more](wsrep-system-index.md#wsrep_sst_receive_address)
...
- **`joiner-timeout-wait-donor-message`**: Determines how long (in seconds) the Joiner waits for the Donor to respond, indicating whether the action to perform is SST or IST. The default value is 60 seconds, which is usually sufficient. If the timeout is reached, the process aborts.

- **`donor-timeout-wait-Joiner`**: Specifies how long (in seconds) the Donor waits for the Joiner to initialize the MySQL instance. The default value is 200 seconds. In slower systems, this value may need to be increased. The Donor log will provide a countdown and indicate if a timeout occurs.

- **`joiner-timeout-clone-instance`**: Sets the time (in seconds) the Joiner waits to detect the MySQL instance where the Clone action will take place. The default value is 90 seconds.
