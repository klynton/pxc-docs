...
* [PXC-](https://perconadev.atlassian.net/browse/PXC-4516): Global Transaction Identifier (GTID) auto-positioning replication didn't work correctly on the Percona XtraDB Cluster node with the new async monitor logic.
...
* [PXC-4585](https://perconadev.atlassian.net/browse/PXC-4585): If the donor node was killed during the State Snapshot Transfer (SST), the SST daemon on the joiner waited indefinitely without terminating. The clone processes continued running until manually killed, and even if the joiner server was killed, the clone daemon kept running until stopped by the user.

* [PXC-4607](https://perconadev.atlassian.net/browse/PXC-4607): The clone plugin remained installed on the joiner node after State Snapshot Transfer (SST).

* [PXC-4609](https://perconadev.atlassian.net/browse/PXC-4609): The clone State Snapshot Transfer (SST) user password was being logged in the error log.

* [PXC-4622](https://perconadev.atlassian.net/browse/PXC-4622): Joining a node with the Clone State Snapshot Transfer (SST) Method failed when using `component_keyring_file-based` encryption.
